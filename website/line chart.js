...
function MultiLine(data) {
  ...
  d3.select("#updateChart").on("click", function () {
    ...
    let selectedAttributes = d3
      .selectAll(".attributeCheckbox:checked")
      .nodes()
      .map(function (d) {
        return d.value;
      });
    ...
    selectedAttributes.forEach(function (attribute) {
      ...
      let text = svg
        ...
        .attr(
          "transform",
          "translate(" +
            x(averages[averages.length - 1][0]) +
            "," +
            y(averages[averages.length - 1][1]) +
            ")"
        )
        .attr("dy", ".35em")
